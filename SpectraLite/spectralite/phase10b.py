"""Phase 10b: OPT-1.3B zero-shot only (no PPL/latency remeasure)."""
...
def run_phase10b_opt13b_zeroshot(
    ...
    *,
    ...
    """Zero-shot lm-eval on dense / ActSVD+gate / Spec-ρ+gate for OPT-1.3B.

    Skips perplexity and latency (already in Phase 10). Order is RAM-friendly:
    ...
    if dense_model is None or tokenizer is None:
        print_section(f"Phase 10b — Load {model_name}")
    ...
    def _eval(model: Any, method: str, notes: str) -> None:
        ...
        out["notes"] = notes
        rows.append(out)
        write_json(f"phase10b_{method}.json", out)
        print_kv(method, f"avg={out.get('zero_shot_avg')}")

    if eval_dense:
        print_section("Phase 10b — Zero-shot dense")
        _eval(dense_model, "dense", f"Phase 10b dense {model_name}")
        _free()
    ...
    if need_acts:
        print_section(
            f"Phase 10b — Calibration (seqs={calib_num_sequences}, "
    ...
    if eval_actsvd_gated:
        ...
        print_section("Phase 10b — Zero-shot ActSVD + gate")
        ...
        _eval(
            ...
            "actsvd_gate_r0.50",
            (
                f"Phase 10b ActSVD+gate ratio={rank_ratio} kappa_speed={kappa_speed} "
    ...
    if eval_spectralite_gated:
        assert activations is not None
        print_section("Phase 10b — Whitened spectral cache + Spec-ρ + gate")
        ...
        _eval(
            ...
            (
                f"Phase 10b Spec-ρ+gate keep={keep_ratio} kappa_speed={kappa_speed} "
                f"replaced={packed['summary']['num_replaced']} "
                f"gated_dense={packed['summary'].get('num_gated_dense', 0)}"
            ),
        )
    ...
    print_section("Phase 10b — Claim snapshot")
    ...
    write_json("phase10b_summary.json", payload)
    write_json("phase10b_table.json", {"table": table, "tasks": list(tasks)})
    write_json("phase10b_claim.json", claim)
    ...
    mark_phase_complete(
        "10b",
        artifacts={
            "summary": "results/phase10b_summary.json",
            "table": "results/phase10b_table.json",
            "claim": "results/phase10b_claim.json",
        ...
        notes=(
            f"OPT-1.3B zero-shot only (dense / ActSVD+gate / Spec-ρ+gate) "
            f"at keep≈{keep_ratio}; complements Phase 10 PPL/latency."
        ),
        config=cfg,
    )
